...
public class EsaminaReport {
		...
		*/
		...
		int size = elencoQuesiti.size();
		 ...
		 while(uscite<size ){
			 ...
			 if (sorgente > 8 && !elencoErrati.isEmpty()){
			 ...
			 System.out.println("copertura :"+elencoUsciti.size()/size+"%"+"("+elencoUsciti.size()+"/"+size+")");
	...
	private static void salva(SortHashtable elenco,String nomeHash) throws Exception {
		...
		oos.close();
		System.out.println("memorizzati "+elenco.size()+" elenco "+nomeHash);
	...
	private static SortHashtable leggi(String nomeHash) throws Exception {
		...
		System.out.println("memorizzati "+oggettoSerializzato.size()+" elenco "+nomeHash);
